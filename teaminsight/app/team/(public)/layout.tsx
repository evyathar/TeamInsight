...
export default function ProtectedTeamLayout({
  ...
  const isTeamHome = path === TEAM_HOME;
  const isTeamJoin = path === TEAM_JOIN;

  function handleBack() {
    if (isTeamHome) {
      router.replace(TEAM_JOIN);
      return;
    }
  ...
  const Icon = isTeamHome || isTeamJoin ? LogOut : ArrowLeft;
  const label = isTeamHome || isTeamJoin ? "Exit" : "Back";
